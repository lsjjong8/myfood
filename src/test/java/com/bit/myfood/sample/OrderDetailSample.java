...
import com.bit.myfood.repository.OrderDetailRepository;
...
//        }
    ...
    private void item(User user){
        ...
        for(int i = 0 ; i < itemCount; i ++){
            // db에 아이템 갯수가 총 x개 ( * 자신의 샘플에 맞추세요 )
            int itemNumber = random.nextInt(40)+1;
        ...
        for(Item item : items){
            ...
            OrderDetail orderDetail = OrderDetail.builder()
                    ...
                    .status(OrderStatus.valueOf(type.equals(OrderType.ALL) ? status :orderDetailStatus))
                    .build();
